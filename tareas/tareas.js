...
const obtenerLista = async () => {
    ...
    tarealist.forEach(lista=>{
        const listadoTarea = document.createElement('li')
        listadoTarea.innerHTML=`<li id=${lista.id} class="todo-item">
        <button class="delete-btn">&#10006;</button>
        <p ${lista.checked ? 'class= "check-todo"' : null}>${lista.text}</p>       
        <button class="check-btn">&#10003;</button>
        </li>
        `;
        lista.appendChild(listadoTarea);
    })
